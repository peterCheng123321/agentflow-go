...
func TestGenerateOpenAICompatResponseCache(t *testing.T) {
	...
	dir := t.TempDir()
	p := NewProvider("qwen-plus", ts.URL, "test-key", BackendOpenAICompat, WithResponseCache(dir, true))
	p.isReady = true

	cfg := GenerationConfig{MaxTokens: 100, Temp: 0.1}
	r1, err := p.Generate("task-a", "ctx-b", cfg)
	if err != nil {
		t.Fatal(err)
	}
	r2, err := p.Generate("task-a", "ctx-b", cfg)
	if err != nil {
		t.Fatal(err)
	}
	if r1 != r2 || r1 != "from-api" {
		t.Fatalf("results: %q %q", r1, r2)
	}
	if calls.Load() != 1 {
		t.Fatalf("expected 1 upstream HTTP call, got %d", calls.Load())
	}
	if p.cacheHits.Load() < 1 {
		t.Fatalf("expected cache hit stat, hits=%d misses=%d", p.cacheHits.Load(), p.cacheMisses.Load())
	}
}
